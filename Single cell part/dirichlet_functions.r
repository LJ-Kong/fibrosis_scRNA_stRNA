...
dirichlet_regression = function(counts, covariates){
	...
	fit = DirichReg(counts ~ tissue + procedure + condition, data)
...
dirichlet_regression_procedure = function(counts, covariates){

    # Calculate regression
    counts = as.data.frame(as.matrix(counts))
    counts$counts = DR_data(counts)
    data = cbind(counts, covariates)
	fit = DirichReg(counts ~ tissue + condition + procedure, data)
    ...
    v = names(pvals)
    pvals = matrix(pvals, ncol=length(u$varnames))
    rownames(pvals) = gsub('procedure', '', v[1:nrow(pvals)])
    colnames(pvals) = u$varnames
    fit$pvals = pvals
    ...
    v = names(coefs)
    coefs = matrix(coefs, ncol=length(u$varnames))
    rownames(coefs) = gsub('procedure', '', v[1:nrow(pvals)])
    colnames(coefs) = u$varnames
    fit$coefs = coefs
...
cell_proportions_tests <- function(freq, notlayer, simple=F) {
	...
	sample2patient <- data.frame(unique(data.frame(sample=meta$sampleid, patient=as.character(meta$patient))), row.names=1)
	sample2procedure <- data.frame(unique(data.frame(sample=meta$sampleid, procedure=meta$procedure)), row.names=1)
...
cell_proportions_procedure <- function(freq, notlayer) {
	# Take only samples from a layer appropriate for the data
	freq.layer <- freq[rownames(freq)%in%meta$sampleid[meta$fraction!=notlayer], ]
	...
	reg <- dirichlet_regression_procedure(counts=freq.layer, covariates=covariates)
...
matrix_barplot = function(data, data.cell, group_by=NULL, pvals=NULL, xlab='', ylab='', value='mean', error='se', legend.show=T, legend.title='Groups', colors='Paired', pos='dodge', border=NA,
    ...
    data = as.data.table(gather(data, 'x', 'y', setdiff(colnames(data), 'group')))
	...
	data.cell <- as.data.table(gather(data.cell, 'x', 'y', setdiff(colnames(data.cell), 'group')))
	...
	if(use_loc & no_heal){
		...
		if(legend.show){
			p = p + theme_bw() +
				scale_fill_manual(values=c(H = "#95d962", N = "#628cd9", F = "#d9628c")) + xlab(xlab) + ylab(ylab)
		}else{
			 p = p + theme_bw() +
				scale_fill_manual(values=c(H = "#95d962", N = "#628cd9", F = "#d9628c"), name=legend.title) + xlab(xlab) + ylab(ylab) +
...
# plot for resection vs biopsy
matrix_barplotx = function(data, data.cell, group_by=NULL, pvals=NULL, xlab='', ylab='', value='mean', error='se', legend.show=T, legend.title='Groups', colors='Paired', pos='dodge', border=NA,
                          out=NULL, nrow=1.5, ncol=1.5, coord_flip=FALSE, sig_only=F, sig_cut=0.05, do.facet=F, pval_cols=c(Over='blue',Under='red'), lab_num_s=3.5, use_loc=F, no_heal=F){
    # Groups (default = rows)
    if(is.null(group_by)){group_by = rownames(data)}
    if(nlevels(group_by) == 0){group_by = as.factor(group_by)}
    ...
    if(!is.null(pvals)){
		...
		#setkeyv(data, c('x', 'group'))
		#setkeyv(pvals, c('x', 'group'))
		data = merge(data, pvals, by.x="x", by.y="group", all=T)
		data$lab1[data$group=="resection"] = ifelse(data$pval[data$group=="resection"] <= .001, '**', ifelse(data$pval[data$group=="resection"] <= .05, '*', ''))
    }
	...
	if(use_loc & no_heal){
		...
		if(legend.show){
			p = p + theme_bw() +
				scale_fill_manual(values=c(biopsy = "orange", resection = "royalblue3")) + xlab(xlab) + ylab(ylab)
		}else{
			 p = p + theme_bw() +
				scale_fill_manual(values=c(biopsy = "orange", resection = "royalblue3"), name=legend.title) + xlab(xlab) + ylab(ylab) +
				theme(legend.position="none")
		}
	}
    # Facet wrap
