...
run.celltype <- function(seur, runlabel, celltype, prefilter_only=F) {
	...
	covariates = data.frame(
		n_genes = scale(seur@meta.data$n_genes_by_counts),
		tissue = factor(seur@meta.data$tissue),
		procedure = factor(seur@meta.data$procedure),
		Layer = factor(seur@meta.data$fraction, levels=c('epi', 'imu')),
		Type = factor(seur@meta.data$status, levels=c('F', 'N', 'H')),
		PubID = factor(seur@meta.data$patient),
		PubIDSample = factor(seur@meta.data$sampleid))
	cv <- covariates[seur@meta.data$annotation2==celltype,,drop=F]
	...
	keep_type <- table(cv$Type) >= 10
	if (any(!keep_type)) {
		if (sum(keep_type) <= 1 || !('F' %in% names(which(keep_type)))) {
			cat(sprintf("Too few cells for comparisons\n"))
			print(table(cv$Type))
			return()
		}
		
		keep <- cv$Type %in% names(which(keep_type))
		cv <- cv[keep,]
		data <- data[,keep]
		
		cat(sprintf("Warning: dropped %s\n", names(which(!keep_type))))
	}
	
	test.contrasts  <- paste('Type', setdiff(names(which(keep_type)), 'F'), sep='')
