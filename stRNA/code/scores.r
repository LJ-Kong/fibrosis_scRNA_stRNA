...
map_names = function(obj=NULL, data=NULL, meta=NULL, names=NULL, regex=NULL, files=NULL, file.cols=NULL, file.regex=NULL, top=NULL, source='human', target='human'){
...
nice_agg = function(x, g, type='median'){
	...
	if(sum(i) == 1){
	    ...
	    if(type == 'mean'){
	        ...
	        sapply(x[i,,drop=F], sd)
	    } else if(type == 'median'){
	        sapply(x[i,,drop=F], median)
	    } else if(type == 'meanmode'){
	        sapply(x[i,,drop=F], meanmode)
	    } else {
	        sapply(x[i,,drop=F], type)
	    }
	} else {
	    rep(NA, ncol(x))
	}
...
nice_agg2 = function(x, g, FUN=sum){
    ...
    g.use = names(which(table(g) == 1))
    print(paste('Skipping', length(g.use), 'rows')); flush.console()
    y = x[g %in% g.use,,drop=F]
    rownames(y) = g[g %in% g.use]
    
    # aggregate remaining rows
    g.use = names(which(table(g) > 1))
    print(paste('Aggregating', length(g.use), 'rows')); flush.console()
    if(length(g.use) > 0){
        z = sapply(g.use, function(gi){
	    i = as.logical(g == gi)
	    i[is.na(i)] = FALSE
	    apply(x[i,,drop=F], 2, FUN)
        })
        z = t(z)
	y = rbind(y, z)
    }
